...
You touch **nothing else**: no other section, no frontmatter (the concept's `generated` and `verified` stamps are not yours), no map state entries, no git commands. You write into the working tree; the sessions own commits, and the next wayfinder session's resolution sweep surfaces your challenges to the operator — re-affirming or amending is theirs, never yours.
