...
5. **Bookkeep** per the reference: Decision and stamp into the waypoint (for a Prototype, that's also when the artifact and its `## Artifact` section are deleted), gist onto its map, sweep fog/blocked/stale prose, cut newly-sharp waypoints, dispatch `atlas:waypoint-researcher` for new research and `atlas:waypoint-prototyper` for new prototypes (brief beneath the Question first) — and **check for quiet**, walking up from the resolved waypoint's map.
