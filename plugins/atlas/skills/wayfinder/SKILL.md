...
**Motions alternate within a session.** A deepen's bookkeeping widens a little every time it cuts newly sharp waypoints or re-fogs; when an answer uncovers a region too foggy to bookkeep in passing, fan out over it properly. A widen never writes a Decision — when the operator wants something resolved that the fan-out surfaced, switch to deepen for that one waypoint: fix it as the focus, run the resolution discipline, stamp, bookkeep, then return to fanning out.
...
4. **Bookkeep** per the reference: Decision and stamp into the waypoint (for a Prototype, that's also when the artifact and its `## Artifact` section are deleted), gist onto its map, sweep fog/blocked/stale prose, cut newly-sharp waypoints and dispatch their agents per the shared flow. Then lint, commit, recommend the next focus, and return to step 2 — the session continues until the operator ends it.
