---
name: widen
description: Use when a project's design bundle needs widening — a loose idea or region spanning many undecided questions, too big or too foggy for a single spec-and-build cycle — including founding a project's first knowledge bundle under design/ and re-mapping a region after implementation reality diverged.
---

# Wayfinding: Widen

Widen the bundle: fan out breadth-first over one region of the project's design bundle, surfacing the open decisions as waypoints and fog that later `/atlas:deepen` sessions resolve. Widening an empty `design/` founds the bundle — naming the root destination is the founding act; there is no separate bootstrap operation.
...
4. **Dispatch background agents.** Plain **unnamed** background Agent calls — never pass `name`; a named (teammate) dispatch silently loses the agent's report. Every research waypoint gets `atlas:waypoint-researcher` launched now, in parallel — pass each agent its waypoint's path; the waypoint's Question is its brief and the reference's sources-and-footnotes contract is its output shape. Every Prototype waypoint gets `atlas:waypoint-prototyper` the same way — its build brief must already sit beneath the Question, written in conversation before dispatch. Neither blocks the session; whatever lands after the session ends waits uncommitted for the next sweep.

5. **Lint, commit, and at the operator's session close: dispatch the auditor, land, stop.** Run `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/lint_bundle.py" design/` and fix every finding first. When the operator ends the session, dispatch `atlas:wayfinding-auditor` — a plain **unnamed** background dispatch, like step 4's — with the bundle root and the concepts this session wrote; a widen resolves nothing, but new waypoints can be stale at birth when they misstate what a sibling settled, and the auditor checks them against the record before anyone builds on them. Then ask the operator to land the session on main — **their typed yes is the approval**; fast-forward merge, push, delete the branch, per the reference's version-control contract. If they defer, push the branch and stop; a PR exists only if they ask for one. Report the region's shape: destination, frontier, what's in the fog, what research is running.
