...
1. **Fix the region and its destination — in conversation, before any file changes.** On a founding session that means the root map's Destination: the project's north star, always ending *no code ships from the bundle* — and the founding act also writes `design/index.md` (the `okf_version` pin) and fixes the operator's actor id in the root map's Notes. On an existing bundle it means naming where in the tree this widening lands and what resolved looks like for that subject. Reflect your understanding back and get the operator's yes — "set this up now" authorizes the edit, not a silent guess at the destination, and everything downstream inherits its scope.
...
3. **Write the map edits.** Cut a working branch from main (or continue the previous session's branch if its PR is still open). Write or extend maps per the reference's node shapes — a subject earns its directory with its first waypoint. For each question sharp enough to state precisely, a waypoint — **one question, one decision, one session's size.** A waypoint that names an *area* ("metering and aggregation") rather than a *decision* ("what is the billable event?") is mis-sized: find the first sharp question inside it and leave the rest as fog. Everything not yet stateable goes to the owning map's Not-yet-specified as loose prose. Wire Frontier and Blocked; Out of scope holds only exclusions the operator has actually ruled. Stamp `generated` on everything you write.

4. **Dispatch research.** Every research waypoint gets `agentic-sdlc:waypoint-researcher` launched now, in parallel — pass each agent its waypoint's path; the waypoint's Question is its brief and the reference's sources-and-footnotes contract is its output shape.

5. **Lint, commit, push, open the PR, stop.** Run `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/lint_bundle.py" design/` and fix every finding first. The PR lands the session's bundle edits on main; **merging is the operator's act, in the Forgejo UI.** Report the region's shape: destination, frontier, what's in the fog, what research is running.
