...
- **Research** — a fact the design waits on, findable in documentation, APIs, or primary sources. Resolved by the `agentic-sdlc:waypoint-researcher` agent, which writes a `## Findings` section plus `sources` frontmatter, each claim carrying a footnote keyed to a source id:
