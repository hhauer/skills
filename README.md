...
Two skills, each invoked as a slash command:

- `/atlas:wayfinder` tends the `design/` bundle in two motions it chooses itself from the bundle's state. Widening fans out over one region of a problem and maps its open questions as *waypoints*, with named fog for what is not yet sharp enough to phrase as a question; it records questions without answering them. Deepening resolves one waypoint at a time in conversation: a decision enters the record only with the reason it was made and a human verification stamp, and the agent cannot resolve a waypoint on its own. Research and prototype waypoints are handed to background agents, so they never block the session, and findings that landed are swept onto the maps before new work starts.
